<?php require("functions.php"); ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>MineWriter</title>
	<?php headIncludes(); ?>
  </head>
  <body>
    <?php navigation(); ?>
    <div class="container">

      <div class="content">
        <div class="page-header">
          <h1>MineWriter <small>Author downloadable books for minecraft!</small></h1>
        </div>            
              <div>
              	<h2 class="big">What is MineWriter?</h2>     
					<ul>
						<li class="big">MineWriter is a service where you can write books for Minecraft!</li>
						<li class="big">This site will be used to generate special text files which can be easily imported into Minecraft.</li>            
						<li class="big">In order to port the books you write into minecraft, you need the BookDownloader plugin.</li>
					</ul>   
				<h2 class="big">Why use MineWriter</h2>     
					<ul>
						<li class="big">You can write extremely large books very easily.</li>
						<li class="big">Your book is saved and you're given a permanent link; your work will never be lost!</li>            
						<li class="big">You can make unlimited copies of your book.</li>
						<li class="big">You can make revisions after it's 'published'.</li>
					</ul>     
				<h2 class="big">When will it launch?</h2>     
					<ul>
						<li class="big">As you can tell, the web development is in it's very early stages.</li>
						<li class="big">The bukkit plugin is also still in development, but it's functional.</li>            
						<li class="big">If we get contributors, it will greatly speed up the process.</li>
						<li class="big">Stay in touch for future updates!</li>
					</ul>              
               </div>
              <div>
              	<h2 class="big">What is BookDownloader?</h2>     
					<ul>
						<li class="big">BookDownloader is the Bukkit plugin that can port website content into books.</li>
						<li class="big">While the plugin itself can parse any document, it defaults to only those from this site.</li>            
						<li class="big">In order to port into Minecraft books, you need this plugin.</li>
					</ul>    
              </div>
            </div>
      </div>
  </body>
</html>
